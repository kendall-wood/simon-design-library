...
export default function ZeteoFeedManager() {
  ...
  const handleFetch = async () => {
    ...
    try {
      const response = await fetch('/api/zeteo/fetch', {
        method: 'POST',
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to fetch Zeteo videos');
      }
    ...
    <div className="w-full min-h-screen bg-white p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          >
            {fetching ? 'Fetching...' : 'Fetch Zeteo Videos'}
          </button>
        </div>
        ...
        <div className="mt-8 p-4 bg-gray-100 border border-gray-300">
          ...
          <ol className="list-decimal list-inside space-y-1 text-xs text-gray-700">
            <li>Click "Fetch Zeteo Videos" to scrape the latest videos</li>
            <li>Videos from the last 7 days will be fetched and stored</li>
            <li>Older videos are automatically removed from the feed</li>
            <li>Videos appear in your main feed mixed with YouTube and Al Jazeera</li>
            <li>Run this daily to keep the feed fresh</li>
          </ol>
          <div className="mt-3 pt-3 border-t border-gray-300">
            <p className="text-xs text-gray-600">
              <strong>Note:</strong> Zeteo videos will have the same autoplay behavior as other videos
